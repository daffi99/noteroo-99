...
export default function CategoryManager({ categories, onCreateCategory, onUpdateCategory, onDeleteCategory, onBack }) {
  ...
  const handleDelete = async (id, name, noteCount) => {
    if (window.confirm(`Are you sure you want to delete category "${name}"? ${noteCount > 0 ? `(${noteCount} note(s) will become uncategorized)` : ''}`)) {
      try {
                  ...
                  {editingId === cat.id ? (
                    ...
                    <div className="category-item-view">
                      ...
                      <div className="category-item-buttons">
                        ...
                        <button className="btn-icon btn-icon--delete" onClick={() => handleDelete(cat.id, cat.name, cat.note_count)} title="Delete category">
                          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
